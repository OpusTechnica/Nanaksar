          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map((item, i) => (
                <div
                  className="p-6 sm:p-7 rounded-2xl bg-white border border-[#0F0F0F]/10 shadow-sm hover:shadow-md hover:border-[#E4A834]/50 transition-all duration-200 max-w-sm w-full flex flex-col justify-between select-none"
                  key={`${index}-${i}`}
                >
                  <div>
                    {/* 5-Star Rating */}
